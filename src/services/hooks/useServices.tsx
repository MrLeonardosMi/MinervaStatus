...
async function logs(key: string): Promise<LogDaySummary[]> {
    const response = await fetch(`https://raw.githubusercontent.com/MrLeonardosMi/MinervaStatus/main/public/status/${key}_report.log`);
...
function fillData(data: LogDaySummary[]): LogDaySummary[] {
    ...
    for (var i = -1; i < DAYS_STATUS-1; i += 1) {
